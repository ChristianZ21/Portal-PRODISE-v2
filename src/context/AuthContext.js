...
// ── Usuario maestro — siempre funciona, sin BD ──────────────
const MASTER = {
  username: 'MASTER',
  password: 'prodise2026',
  perfil: { username: 'MASTER', nombre: 'Administrador del Sistema', nivel: 1, dni: null }
}
...
export function AuthProvider({ children }) {
  ...
  async function login(username, password) {
    ...
    // 1. Usuario maestro — sin tocar la BD
    ...
    try {
      ...
      // Consulta simple — SIN joins para evitar errores de FK
      ...
      if (usr.password_hash !== hash) {
        return { success: false, error: 'Contraseña incorrecta' }
      }

      // Obtener nombre del trabajador vinculado (consulta separada)
      let nombre = uUp
      if (usr.dni_asociado) {
...
// ── MD5 en JavaScript puro ──────────────────────────────────
// Mismo resultado que Python: hashlib.md5("texto").hexdigest()
function md5(input) {
  ...
  const s8 = unescape(encodeURIComponent(input))
  const x = []
  for (let i = 0; i < s8.length; i += 4)
    x[i >> 2] = s8.charCodeAt(i) + (s8.charCodeAt(i+1) << 8) +
                (s8.charCodeAt(i+2) << 16) + (s8.charCodeAt(i+3) << 24)
  const l = s8.length
  x[l >> 2] |= 0x80 << ((l % 4) * 8)
  x[(((l + 64) >>> 9) << 4) + 14] = l * 8
